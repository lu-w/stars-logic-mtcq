...
class MTCQEvaluator<
    ...
    fun eval(segment: S, mtcqString: String, dlConvertibleClasses: Set<Any> = setOf()): QueryResult {
        val tkb = toTKB(segment, dlConvertibleClasses)
    ...
    private fun toTKB(segment: S, dlConvertibleClasses: Set<Any> = setOf()): TemporalKnowledgeBase {
        // Check cache
        ...
        segment.tickData.forEach { tick ->
            ...
            for (prop in (tick::class as KClass<T>).memberProperties) {
                ...
                if (data is Iterable<*>)
                    data.forEach { entity ->
                        if (entity != null && entity.isDLConvertible(dlConvertibleClasses))
                            entity.addToKB(kb, prefix, dlConvertibleClasses = dlConvertibleClasses)
                    }
                if (data != null && data.isDLConvertible(dlConvertibleClasses))
                    data.addToKB(kb, prefix, dlConvertibleClasses = dlConvertibleClasses)
            }
            tkb.add(kb)
        }
        // Update cache
